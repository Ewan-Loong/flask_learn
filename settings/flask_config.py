...
class BaseConfig:
    ...
    PERMANENT_SESSION_LIFETIME = timedelta(hours=10)  # Session的生命周期(秒) 默认10min的秒数
    ...
    # JSON_AS_ASCII = False　高版本舍弃　app.json.ensure_ascii = False 代替
    # 默认情况下 Flask 使用 ascii 编码来序列化对象。如果这个值被设置为 False ，
    # Flask不会将其编码为 ASCII，并且按原样输出，返回它的 unicode 字符串。
    # 比如 jsonfiy 会自动地采用 utf-8 来编码它然后才进行传输。

    # JSON_SORT_KEYS = True　高版本舍弃 app.json.sort_keys = True 代替
...
DevelopmentConfig.DEBUG = True  # 是否开启Debug模式，开启编辑时代码重启，LOG打印级别最低，错误信息透传

# 测试环境
TestingConfig = BaseConfig()
TestingConfig.TESTING = True  # 是否开启测试模式，无限接近生产环境，代码编辑时不会重启，LOG级别较高，错误信息不再透传

# 生产环境
ProductionConfig = BaseConfig()
ProductionConfig.TESTING = True  # 是否开启测试模式，无限接近生产环境，代码编辑时不会重启，LOG级别较高，错误信息不再透传
